    ...
    def on_event(self, response: str) -> None:
        try:
            ...
            if 'response.audio.delta' == event_type:
                ...
                # 时长(秒) = 字节数 / (采样率 * 声道数 * 位深度/8)
                # 16000 * 1 * 2 = 32000 字节/秒
                audio_duration = len(audio_bytes) / 32000.0
                self.node.get_logger().info(f"Audio packet duration: {audio_duration:.4f} seconds")
...
class ROSOmniExpressionCallback(OmniRealtimeCallback):
    """
    重写回调类，仅处理表情输出
    """
    def __init__(self, node_instance):
    ...
    def on_open(self) -> None:
        self.node.get_logger().info('✅ DashScope Expression Connection opened!')

    def on_close(self, close_status_code, close_msg) -> None:
        self.node.get_logger().warn(f'⚠️ DashScope Expression Connection closed: {close_msg}')
        # 更新连接状态为已关闭
        self.node.omni_expression_connected = False

    def on_event(self, response: str) -> None:
        try:
            event_type = response.get('type','unknown')
            print(f">>>[DEBUG] Expression Model Recieved Event:{event_type}", flush=True)

            if event_type=='error':
                self.node.get_logger().error(f"EXPRESSION SERVER ERROR:{response}")
                # 遇到错误时，更新连接状态为已关闭
                self.node.omni_expression_connected = False
            
            # 处理表情相关的事件
            # 这里需要根据实际的表情输出事件类型进行调整
            # 假设表情输出事件类型为 'response.expression.delta'
            elif 'response.text.delta' == event_type:
                expression_data = response.get('delta', {})
                self.node.get_logger().info(f"[[[response.text.delta Emotion AI]]]: {expression_data}")
                
                # # 发布情绪数据
                # emotion_msg = String()
                # emotion_msg.data = expression_data
                # self.node.pub_emotion.publish(emotion_msg)
            
            # 处理完整的表情回复
            #elif 'response.text.completed' == event_type:
            elif 'response.text.done' == event_type:
                expression_data = response.get('text', {})
                if expression_data:
                    self.node.get_logger().info(f"[[[response.text.done Emotion AI]]] (completed): {expression_data}")
            ...
            self.node.get_logger().error(f"Error in expression callback: {e}")
...
class OmniBrainNode(Node):
    def __init__(self):
        ...
        self.omni_expression_connected = False
    ...
    def init_omni_connection(self):
        try:
            ...
            # --- 连接第二个只输出表情的模型 --- 
            self.get_logger().info("Connecting to expression-only model...")
            expression_callback = ROSOmniExpressionCallback(self)
            
            self.expression_conversation = OmniRealtimeConversation(
                model='qwen3-omni-flash-realtime',
                callback=expression_callback, 
            )

            # 连接到 DashScope
            self.get_logger().info("Attempting to connect to DashScope for expression model...")
            expression_connect_result = self.expression_conversation.connect()
            self.get_logger().info(f"Expression model Connect() returned: {expression_connect_result}")
            
            # 检查连接状态
            expression_connection_status = getattr(self.expression_conversation, 'connected', None)
            if expression_connection_status is None:
                self.get_logger().warn("Expression model connection status attribute not found, using alternative check")
                # 由于无法直接获取连接状态，我们假设连接成功，后续会通过错误处理来更新状态
                self.omni_expression_connected = True
            elif expression_connection_status:
                self.get_logger().info("✅ Expression model WebSocket connection established successfully")
                self.omni_expression_connected = True
            else:
                self.get_logger().error("❌ Expression model WebSocket connection failed")
                self.omni_expression_connected = False
            
            # 等待连接完全建立
            self.get_logger().info("Waiting for expression model connection to fully establish...")
            time.sleep(2.0)
            self.get_logger().info("Expression model connection setup completed")
            # 配置 Session 参数，只输出表情
            self.expression_conversation.update_session(
                ...
                enable_turn_detection=True,
                turn_detection_type='server_vad',
                instructions="你是一个只会回答“happy”、“sad”、“nature”、“amazing”的机器，请感受用户的话语和语气，并返回你听到后你自己的情绪，从“happy”、“sad”、“nature”、“amazing”中选择一个。"
            )
            self.get_logger().info("DashScope Expression Session Initialized.")
    ...
    def audio_input_callback(self, msg: AudioData):
        ...
        try:
            # 检查连接状态
            if not self.omni_connected or not self.omni_expression_connected:
            ...
            for i in range(0, total_len, self.AUDIO_CHUNK_SIZE):
                ...
                try:
                    ...
                    # 发送给第二个模型（仅表情输出）
                    if self.omni_expression_connected:
                        self.expression_conversation.append_audio(audio_b64)
            ...
            if is_first_audio and self.image_buffer is not None:
                try:
                    ...
                    if result:
                        ...
                        # 发送给第二个模型（仅表情输出）
                        if self.omni_expression_connected:
                            self.expression_conversation.append_video(img_b64)
