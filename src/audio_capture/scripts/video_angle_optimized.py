    ...
    def compute_motion_saliency(self, current_frame: np.ndarray) -> np.ndarray:
        ...
        if self.prev_gray is not None:
            if self.use_background_subtraction:
                ...
                # 转换为浮点
                motion_saliency = fg_mask.astype(np.float32) / 255.0
            else:
                # 方法2：简单帧差 + 形态学处理
                frame_diff = cv2.absdiff(current_gray, self.prev_gray)
                # 二值化
                _, binary = cv2.threshold(frame_diff, 25, 255, cv2.THRESH_BINARY)
                # 形态学操作去噪
                kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
                binary = cv2.morphologyEx(binary, cv2.MORPH_OPEN, kernel)
                binary = cv2.morphologyEx(binary, cv2.MORPH_CLOSE, kernel)
                motion_saliency = binary.astype(np.float32) / 255.0
            
            # 应用高斯滤波平滑
            motion_saliency = cv2.GaussianBlur(motion_saliency, (7, 7), 1.5)
...
class CameraNode(Node):
    def __init__(self):
        ...
        self.motion_detector = MotionDetector(use_background_subtraction=True)
        
        cv2.namedWindow("Motion Detection System (Optimized)", cv2.WINDOW_NORMAL)
    ...
    def main_loop(self):
        print("=" * 60)
        print("运动目标检测系统已启动 (优化版本)")
        print("=" * 60)
        print("算法: MOG2 背景减除 (比 SIFT 快 10x+)")
        print("硬件: CPU 优化 (无 GPU 加速)")
        print("按 'Esc' 键退出程序")
        print("按 's' 键保存当前结果")
        print("=" * 60)
        ...
        while rclpy.ok():
            ...
            cv2.imshow("Motion Detection System (Optimized)", display_image)
