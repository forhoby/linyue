...
#lix_added
def init_motors(port, motor_list):
    serial_dev = serial.Serial(port, 921600, timeout=0.5)
    ctrl = MotorControl(serial_dev)

    for m in motor_list:
        ctrl.addMotor(m)
        ctrl.switchControlMode(m, Control_Type.MIT)

    return ctrl, serial_dev

def enable_motors(ctrl, motors):
    for m in motors:
        ctrl.enable(m)
    sleep(0.05)

def refresh_all(ctrl, motors):
    for m in motors:
        ctrl.refresh_motor_status(m)

def disable_all(ctrl, motors):
    for m in motors:
        ctrl.disable(m)
